...
class AdminReportController extends Controller {
  ...
  public function overview(Request $req) {
    ...
    if ($courseId) $enrollQ->where('course_id', $courseId);
    ...
    $daily = LessonProgress::query()
      ...
      ->when($courseId, function($q) use ($courseId) {
        $q->whereIn('lesson_id', Lesson::whereIn('module_id', Module::where('course_id',$courseId)->pluck('id'))->pluck('id'));
      })
  ...
  public function students(Request $req) {
    ...
    foreach ($page->items() as $u) {
      ...
      $computedStatus =
        !$startedFlag ? 'idle' :
        ($this->isCourseCompletedForUser($u->id, $courseId) ? 'completed' : 'started');

      if ($status && $computedStatus !== $status) continue;
  ...
  public function studentProgress(Request $req, int $userId) {
    ...
    if (!$courseId) return response()->json(['message'=>'course_id required'], 422);

    $course = Course::with(['modules' => function($q){ $q->orderBy('position'); },
                            'modules.lessons' => function($q){ $q->orderBy('position'); }])
      ->findOrFail($courseId);

    $lessonIds = [];
    foreach ($course->modules as $m) foreach ($m->lessons as $l) $lessonIds[] = $l->id;
    ...
    foreach ($course->modules as $m) {
      foreach ($m->lessons as $l) {
        ...
        if ($status === 'completed') $done[] = $row;
        else $outstanding[] = $row;
      }
    }
  ...
  private function countIdleEnrolled(?int $courseId): int {
    ...
    if ($enrolledUserIds->isEmpty()) return 0;
  ...
  private function countStartedEnrolled(?int $courseId): int {
    ...
    return LessonProgress::whereIn('user_id',$enrolledUserIds)
      ...
      ->select('user_id')->distinct()->count('user_id');
  }
  ...
  private function isCourseCompletedForUser(int $userId, int $courseId): bool {
    ...
    $c = LessonProgress::where('user_id',$userId)->whereIn('lesson_id',$lessonIds)->where('status','completed')->count();
    return $c >= $total;
  }
}
